...
class CBAM_PretrainedResNet18(nn.Module):

    def __init__(self, dropout_rate=0.3):
        super(CBAM_PretrainedResNet18, self).__init__()

        # loading pre_trained resnet
        self.resnet = resnet18(weights=ResNet18_Weights.DEFAULT)

        # adding CBAM blocks after each residual stage
        self.resnet.layer1 = nn.Sequential(self.resnet.layer1, CBAMLayer(64))
        self.resnet.layer2 = nn.Sequential(self.resnet.layer2, CBAMLayer(128))
        self.resnet.layer3 = nn.Sequential(self.resnet.layer3, CBAMLayer(256))
        self.resnet.layer4 = nn.Sequential(self.resnet.layer4, CBAMLayer(512))
        ...
        # unfreezing all CBAM Layers
        for layer in [self.resnet.layer1, self.resnet.layer2, self.resnet.layer3, self.resnet.layer4]:
            for module in layer.modules():
                if isinstance(module, CBAMLayer):
                    for param in module.parameters():
                        param.requires_grad = True
